...
Person.ransack(brand_in: ['AUDI']).result.to_sql
# ... WHERE "people"."brand" IN ('AUDI')

Person.ransack(brand_in: ['AUDI', '__ransack_null__']).result.to_sql
# ... WHERE ("people"."brand" IN ('AUDI') OR "people"."brand" IS NULL)

Person.ransack(brand_in: ['__ransack_null__']).result.to_sql
# ... WHERE "people"."brand" IS NULL
```
...
```ruby
Person.ransack({ brand_in: ['AUDI', '__ransack_null__'] }, null_sentinel: '__ransack_null__')

# With a global null_sentinel configured, turn it off for one search:
Person.ransack({ brand_in: ['AUDI', '__ransack_null__'] }, null_sentinel: false)
```
